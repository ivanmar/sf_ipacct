...
var GLS;	// GLS means "GoLoginStatus"
var blur = 0;
...
var close_message = "You are now LOGGED OFF";

function handler() {
	if (handler.arguments.length==0) return null;

	context = handler.arguments[0];	
	if (handler.arguments[1]) mytimeleft = handler.arguments[1];
	if (handler.arguments[2]) next_url = handler.arguments[2];
	switch ( context ) {	
...
function success(url,userurl) {
	...
	popUpWindow(url,'GLS','272','212',0,0,0,0);
...
function failed() {
	...
	if (self.name != 'GLS') {
		popUpWindow('','GLS','272','212',0,0,0,0);	// GLS means "GoLoginStatus"
		closeGLP();
	}
}
...
function forcelogoff(logoffurl) {
	window.location.href = logoffurl; 
        
	var LOGOUT = window.open(logoffurl,'','width=1,height=1,left=0,top=0,scrollbars=no');
        if(LOGOUT) {
             LOGOUT.close();
        } else if (window.opener && !LOGOUT) {
             opener.location.href = logoffurl;
	}
	window.close();
}
